...
def removeChair(cursor=None, chairID=None):
    if not checkCursor(cursor):
        print("Set cursor variable")
        return False
    if isinstance(chairID, int) and isChairExists(cursor=cursor, chairID=chairID):
...
def removeDirection(cursor=None, directionID=None):
    if not checkCursor(cursor):
        print("Set cursor variable")
        return False
    if isinstance(directionID, int) and isDirectionExists(cursor=cursor, directionID=directionID):
