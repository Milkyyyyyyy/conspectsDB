    ...
    try:
        while True:
            ...
            if previous_message_id:
                await bot.edit_message_text(chat_id=chat_id, message_id=previous_message_id, text=text,
                                            parse_mode='HTML')
            ...
            if 'page' in response:
                ...
                try:
                    ...
                    await delete_message_after_delay(bot, chat_id=chat_id, message_id=previous_message_id,
                                                     delay_seconds=1)
    finally:
        awaiters.pop(key, None)
...
async def wait_for_any_button(
    ...
    key = (user_id, chat_id)
    if key in awaiters or not awaiters[key].done():
        raise RuntimeError('Already waiting for a response from the user')
    ...
    try:
        ...
        await send_temporary_message(bot, chat_id=chat_id, text='Вы ничего не выбрали', delay_seconds=5)
        async with bot.retrieve_data(user_id=user_id, chat_id=chat_id) as data:
            data.pop('waiting_for', None)
        return None
    finally:
