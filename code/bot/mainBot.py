...
from code.database.classes import namespaced
...
async def change_facult_page(call):
    ...
    if new_page <= 0:
        new_page = 1
    await process_facult(page=new_page, userID=call.from_user.id, chatID=call.message.chat.id, previousMessageID=call.message.id)
...
async def accept_registration(call):
    await bot.answer_callback_query(call.id)
    try:
        await bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
    except Exception:
        pass
    bot.set_state(call.from_user.id, MenuStates.main_menu, call.message.chat.id)
